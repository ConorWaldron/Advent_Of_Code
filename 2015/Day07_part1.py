...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        ...
        parsed_instructions, set_of_unknown_variables, known_variable_dict = instruction_reader(circuit_instructions)
        
        counter = 0
        while 'a' in set_of_unknown_variables:
            print(f'Loop number {counter}, there are {len(set_of_unknown_variables)} unknown variables and {len(parsed_instructions)} commands left to process')
            test_possibles = find_possible_cmds(parsed_instructions, known_variable_dict)
            parsed_instructions, set_of_unknown_variables, known_variable_dict = execture_cmd(parsed_instructions, test_possibles, set_of_unknown_variables, known_variable_dict)
            counter += 1
        
        end_time = time.time()
        duration = end_time - start_time
        print('The value of a is {}'.format(known_variable_dict['a']))
